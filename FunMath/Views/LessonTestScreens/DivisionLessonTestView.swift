...
struct DivisionLessonTestView: View {
    @State private var firstNumber: Int = Int.random(in: 100...999)
    @State private var secondNumber: Int = Int.random(in: 1...99)
    ...
    private func checkAnswer() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
            self.resetGame()
            if testCounter < totalTests {
                self.firstNumber = Int.random(in: 100...9999)
                self.secondNumber = Int.random(in: 1...99)
                self.setOptions()
...
struct DivisionLessonTestView_Previews: PreviewProvider {
    static var previews: some View {
        DivisionLessonTestView()
    }
}
